...
> **Note:** This document is a superseded draft. The current source of truth is `PRD.md`.
> Differences from `PRD.md`: this version describes graph injection into Neo4j as in-scope for MVP
> (deferred per PRD), and references the old `google-generativeai` package (migrated to `google.genai`).
